...
{
public:
	void rayThread(Image &result, int x_begin, int x_eind, Vec3Df origin10, Vec3Df origin00, Vec3Df origin01, Vec3Df dest10, Vec3Df dest01, Vec3Df dest00);
};
...
void rayThread(Image &result, int x_begin, int x_eind, Vec3Df origin10, Vec3Df origin00, Vec3Df origin01, Vec3Df dest10, Vec3Df dest01, Vec3Df dest00)
{
	...
	// 4 = multisampling x16
	int samplinglevel = 1;
	int samples = 1; // pow(2, samplinglevel);

	// IF YOU WANT TO DISABLE MULTISAMPLING //
	//int samplinglevel = 1;
	//int samples = 1;
	for (unsigned int y = 0; y<WindowSize_Y; ++y) {
		for (unsigned int x = x_begin; x<x_eind; ++x) {
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'r':
	{
				...
				//number of threads you want to run with
				int parts = 8;
				...
				for (int i = 0; i < parts - 1; ++i) {
					tt.push_back(std::thread(rayThread, std::ref(result), bnd[i], bnd[i + 1], origin10, origin00, origin01, dest10, dest01, dest00));
				}

				//Use the main thread to do part of the work !!!
				int i = parts - 1;
				rayThread(result, bnd[i], bnd[i + 1], origin10, origin00, origin01, dest10, dest01, dest00);
